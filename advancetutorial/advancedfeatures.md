...
既然我们拥有了用于抓取所有内容的所有构建模块，仍然存在的问题：我们如何将它们放在一起？ 如果没有Prefect，则可以考虑编写循环遍历**create_episode_list**结果。对于每个情节，可以将所有内容放在**try/except**块中，以捕获任何错误并让其他情节抓取继续进行。 如果你随后想要利用并行性，则需要考虑到还不知道**create_episode_list**将返回多少集，因此需要进一步重构。
